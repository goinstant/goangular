...
Connection.prototype.$loginUrl = function(providers, returnTo) {
  ...
  if (_.isArray(providers)) {
    _.each(providers, function(provider) {
      self.loginProviders[provider] =
        self.$$conn.loginUrl(provider.toLowerCase(), returnTo);
    });

  } else {
    var provider = providers ? providers : 'default';

    this.loginProviders[provider] =
      this.$$conn.loginUrl(provider.toLowerCase(), returnTo);
  }
};
